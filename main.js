...
// funcion para mostrar resultados dentro del DOM
function mostrarResultados(resultado) {
    // verifico que el arreglo contenga elementos
...
// funcion para buscar por título del libro 
function buscarTitulo(titulo) {
    const resultado = libros.filter((libro) => libro.titulo.toLowerCase().includes(titulo));
    mostrarResultados(resultado);
}

// funcion para buscar por autor del libro 
function buscarAutor(autor){
    const resultado = libros.filter((libro) => libro.autor.nombre.toLowerCase().includes(autor));
    mostrarResultados(resultado);
}


function realizarBusqueda(opcionIngresada){
    switch (opcionIngresada) {
        case '1':
            tituloBuscado = prompt("Ingrese título de libro");
            buscarTitulo(tituloBuscado);
            break;
        case '2':
            autorBuscado = prompt("Ingrese autor del libro");
        ...
        case '3':
            ...
            break;
        default:
            alert('La opción ingresada no corresponde a ninguna opción válida');
            opcionIngresada = prompt(mensaje);
            iniciarMenu(opcionIngresada);
            break;
    }
}
